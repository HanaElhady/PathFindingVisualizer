export const MAX_ROWS = 29;
export const MAX_COLS = 39;
export const SPEED = 0.5;

export const START_TILE_CONFIGURATION = {
  row: 1,
  col: 1,
  isEnd: false,
  isWall: false,
  isPath: false,
  distance: 0,
  isStart: false,
  isTraversed: false,
  parent: null,
};

export const END_TILE_CONFIGURATION = {
  row: MAX_ROWS - 2,
  col: MAX_COLS - 2,
  isEnd: false,
  isWall: false,
  isPath: false,
  distance: 0,
  isStart: false,
  isTraversed: false,
  parent: null,
};

// export const TILE_STYLE =
//   "lg:w-[17px] md:w-[15px] xs:w-[8px] w-[7px] lg:h-[17px] md:h-[15px] xs:h-[8px] h-[7px] border-t border-r border-pink-200";
// export const TRAVERSED_TILE_STYLE = TILE_STYLE + " bg-cyan-500";

// export const START_TILE_STYLE = TILE_STYLE + "";//start
// export const END_TILE_STYLE = TILE_STYLE + " bg-green-600";//end

// export const WALL_TILE_STYLE = TILE_STYLE + " bg-gray-200";

// export const PATH_TILE_STYLE = TILE_STYLE + "👣";//path


export const SLEEP_TIME = 5;
export const EXTENDED_SLEEP_TIME = 30;
